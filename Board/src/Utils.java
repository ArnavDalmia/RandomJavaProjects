 ...
 */
	public static int obtainIntegerInputLowHigh(String prompMessage, int low, int high) {
		...
		if ((choice < low) || (choice > high)){												// checks if the choice variable is in between low and high, and if it isn't
			System.out.println("Invalid Choice Entered. Try Again!");						// there will be an error message that shows
		}
...
/*Method optainInput
 * This method is used to obtain a user input of a string
 * 
 * Parameters:
 * 			msg :	String message that is asked of the user so that they can input based on the message
 * 
 * Return Values:
 * 			choice :	String variable that contains the user inputted response to msg 
 * 
 * Local Variables Used:
 * 			choice :	String variable that contains the user inputted response to msg
 */
	public static String optainInput(String msg) { 
		System.out.println(msg); 											// prints msg from parameter
		Scanner userInput = new Scanner(System.in); 						// initializes instance of Scanner class
		String choice; 														// creates empty String varaible that will hold the user input
		choice = userInput.nextLine(); 										// stores user input within variable choice
		return choice; 														// returns the value of the user input, choice variable
	}
